...
qos_policies = {
    ...
    'slither': {
        ...
        'rfc4594_class': 'Real-Time Interactive'
    },
    ...
    'youtube': {
        ...
        'rfc4594_class': 'Multimedia Streaming'
    },
    ...
    'dropbox': {
        ...
        'rfc4594_class': 'High-Throughput Data'
    },
    ...
    'trello': {
        ...
        'latency_sensitivity': 'medium',
        'jitter_tolerance': 'high',
        'rfc4594_class': 'Transactional Data'
    },
...
def save_and_show_plot(fname):
    ...
    plt.show(block=True)  # waits for user to close window before continuing
    plt.close('all')       # clean up to prevent tkinter threading errors
...
def preprocess(data):
    ...
    plt.tight_layout()
    save_and_show_plot("traffic_distribution.png")
...
def train_model(X, y):
    ...
    # ---- Step 1: Train/Test Split FIRST (reviewer requirement) ----
    # Split on raw unscaled features so nothing leaks into the test set
    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=0.2, stratify=y, random_state=42
    )

    # ---- Step 2: Fit StandardScaler on TRAINING SET ONLY ----
    # Never fit on the full dataset — that would leak test set statistics
    ...
    # ---- Step 3: Oversampling on TRAINING SET ONLY ----
    ...
    if oversampling_method == "SMOTE":
        ...
        print(f"Applied SMOTE to train set: resampled to {X_train_res.shape}")
    elif oversampling_method == "Random Oversampling":
        ros = RandomOverSampler(random_state=42)
        X_train_res, y_train_res = ros.fit_resample(X_train_scaled, y_train)
        print(f"Applied Random Oversampling to train set: resampled to {X_train_res.shape}")
    else:
    ...
    if oversampling_method:
        ...
        print(pd.Series(y_train_res).value_counts().head(10))

    # ---- Step 4: LDA fit on TRAINING SET ONLY, transform both sets ----
    if use_lda:
        n_components = min(len(y.unique()) - 1, X_train_res.shape[1], 50)
        lda = LinearDiscriminantAnalysis(n_components=n_components)
        X_train_final = lda.fit_transform(X_train_res, y_train_res)   # fit + transform train
        X_test_final  = lda.transform(X_test_scaled)                  # transform test only
        print(f"Applied LDA: {X_train_res.shape} ...features reduce to {X_train_final.shape}")
        X_train_final = pd.DataFrame(X_train_final, columns=[f'LDA_{i+1}' for i in range(X_train_final.shape[1])])
        X_test_final  = pd.DataFrame(X_test_final,  columns=[f'LDA_{i+1}' for i in range(X_test_final.shape[1])])
    ...
    return rf_model, X_test_final, y_test, scaler
...
def qos_chart_visualizations(y, priority_counts, latency_counts):
    ...
    plt.tight_layout()
    save_and_show_plot("priority_distribution.png")
    ...
    plt.legend(handles=legend_patches, loc='upper right')
    plt.tight_layout()
    save_and_show_plot("latency_sensitivity.png")
    ...
    plt.legend(handles=legend_patches, loc='upper right')
    plt.tight_layout()
    save_and_show_plot("bandwidth_requirements.png")
...
def evaluate_model_with_qos(model, X_test, y_test, y):
    ...
    plt.tight_layout()
    save_and_show_plot("confusion_matrix.png")

    #top features identified by RF
    if use_lda == False:
...
try:
    ...
    print(f"Total execution time: {minutes:.0f} minutes and {seconds:.2f} seconds")
